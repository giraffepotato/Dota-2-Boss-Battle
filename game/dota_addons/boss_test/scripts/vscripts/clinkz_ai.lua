...
function Spawn(entityKeyValues)
	ABILITY_revive_clinkz_spell = thisEntity:FindAbilityByName("boss_revive_clinkz")
...
local healthRemaining = thisEntity:GetHealth() / thisEntity:GetMaxHealth()
	if ABILITY_revive_clinkz_spell:IsFullyCastable() then
		...
		detected = "false"
		--like in the other one the string is b/c im too lazy to bool concat 4Head
		for i=1,#units,1 do
			local i_unit = units[i]:GetUnitName()
			if units ~= nil and i_unit ~= nil then 
				if i_unit == "npc_drow_ranger_boss" then
					detected = "true"
				end
			end
		end
		if not (detected == "true") then
			thisEntity:CastAbilityNoTarget(ABILITY_revive_clinkz_spell, index)
		end
	end	 
